...
class dtw(object):
	...
	def compute_location(self,W):
		Na,Nb = W.shape
		mu = np.zeros((Na+1,Nb+1))
		mu[0,:] = -np.inf
		mu[:,0] = -np.inf
		mu[0,0] = 0
		for i in range(1,Na+1):
			for j in range(1,Nb+1):
				mu[i,j]  = logsumexp([mu[i-1,j]+self.alpha*W[i-1,j-1],mu[i,j-1]+self.alpha*W[i-1,j-1],mu[i-1,j-1]+self.alpha*W[i-1,j-1]])
		return mu

	def compute_transition(self,W,reduced):
		Na,Nb = W.shape
		mu = self.compute_location(W)
		pi = np.zeros((Na+2,Nb+2,3))
		pi[0,0] = np.array(([0,0,1]))
		pi[-1,-1]  = np.array(([0,0,1]))
		for i in reversed(range(1,Na+1)):
			for j in reversed(range(1,Nb+1)):				
				log_pi = np.array((mu[i-1,j]+self.alpha*W[i-1,j-1],mu[i,j-1]+self.alpha*W[i-1,j-1],mu[i-1,j-1]+self.alpha*W[i-1,j-1]))-mu[i,j]
	...
	def compute_omega(self,W):
		...
		for i in range(1,Na+1):
			for j in range(1,Nb+1):
				lam[i,j] = np.array(([lam[i-1,j],lam[i,j-1],lam[i-1,j-1]]))@pi[i,j]
		
		# Reversed iter for rho
		for i in reversed(range(Na)):
			for j in reversed(range(Nb)):
				rho[i,j] = np.array(([rho[i+1,j],rho[i,j+1],rho[i+1,j+1]]))@np.array(([pi[i+2,j+1,0],pi[i+1,j+2,1],pi[i+2,j+2,2]]))
				# print(i,j)
		...
		for i in range(1,Na+1):
			for j in range(1,Nb+1):
				omega[i,j] = np.array(([lam[i-1,j],lam[i,j-1],lam[i-1,j-1]]))*pi[i,j]*rho[i-1,j-1]
