...
class MultiHeadScaledProductAttention(BaseAttention):
    ...
    def forward(self, inputs, memory, memory_lengths=None, query_lengths=None, causality=None,mask = None):
        ...
        if mask is not None:
            length_mask = mask.unsqueeze(1)
        # [batch, num_head, q_max_time, m_max_time]
        paddings = torch.ones_like(logits, dtype=logits.dtype) * (-2. ** 32 + 1)
...
class CrossAttentionBlock(nn.Module):
    def __init__(self, input_dim, memory_dim, attention_dim, attention_heads, attention_temperature,
        ...
        # self.att_proj1 = spectral_norm(LinearNorm(attention_dim + input_dim, input_dim))
        ...
        # self.att_proj2 = spectral_norm(LinearNorm(attention_dim * 2, attention_dim))
